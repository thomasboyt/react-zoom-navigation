          ...
          <TransitionGroup className="transition-group" component={null}>
            <Transition key={location.key} timeout={ANIMATION_TIME_MS}>
              {state => {
                ...
                // TODO: Do something else to match this...
                const matches = location.pathname.startsWith(
                  "/" + this.props.for
                );

                // transition is active for this path
                if (matches) {
                  // card detail page
                  return (
                  ...
                  // base card
                  // TODO: only animate if coming from correct card....
                  return (
                    <BaseCardTransition state={state}>
                      {inner}
                    </BaseCardTransition>
                  );
                }
